                ...
                if (league_id == 0)
                {
                    if (division_id == 0)
                    {
                        cadena = "SELECT p.ID, c_local.nombrecompleto, c_visita.nombrecompleto, p.golesLocal, p.golesVisita, e_local.zona FROM partido as p " +
                                ...
                                "INNER JOIN fecha as f ON f.ID = p.fechaLiga WHERE f.numero = '" + matchday_id.ToString() + " AND (e_local.division = 4 or e_local.division = 5 or e_local.division = 16 or e_local.division = 17 or e_local.division = 15 or e_local.division = 20) ORDER BY c_local.ID, p.ID;";
                    }
                    else if (division_id == 1)
                    {
                        cadena = "SELECT p.ID, c_local.nombrecompleto, c_visita.nombrecompleto, p.golesLocal, p.golesVisita, e_local.zona, e_local.division," +
                            ...
                            "ORDER BY  c_local.ID, c_visita.id, p.ID, orden_personalizado";
                    }
